...
for(k in ks) {
  ...
  for(j in 1:num_run) {
    cat(paste("k:", k, "j:", j, "\n"))
    res <- stm(poliblog5k.docs, poliblog5k.voc, K=k, prevalence=~rating, data=poliblog5k.meta, max.em.its = 100, verbose = FALSE, control=list(method="BFGS", neum_sum_cpp=TRUE))
...
#print(neum_sum_df)
save(neum_sum_df,file="/home/gomfy/sociology/stm/git/sprint/stm/sprint/neum_sum.Rda")
