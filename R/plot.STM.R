...
plot.STM <- function(x, 
  ...
  if(type=="summary") {
    frequency <- colMeans(model$theta)
  ...
  if(type=="perspectives") {
    ...
    # this basically has the effect of flattening it out a bit so the gian things aren't huge.
    ...
    plot(c(0,0), xlim=c(min(diff) - .1, max(diff)+.1), ylim=c(-2*length(diff),6*length(diff)), type="n", 
         xaxt="n", xlab="", main="", yaxt="n", ylab="", bty="n")
    ...
    text(diff, rand, model$vocab[words], cex=scale, col=colors)
