...
multiSTM <- function(mod.out, ref.model=NULL, 
                     align.global=FALSE, mass.threshold=1, 
                     reg.formula=NULL, metadata=NULL, reg.nsims=100,
                     reg.parameter.index=2, verbose=TRUE){
  if(align.global && !require(clue)) stop("Please install the clue package to use align.global=TRUE")
  # Catch bad inputs
  if (is.null(mod.out$runout))
    stop("The mod.out argument must be a list of the type generated by selectModel().")
  ...
  if(length(mod.out$runout[[1]]$beta$logbeta)>1) stop("multiSTM does not yet work with content covariates.")
  
  # Number of models in object
  N <- length(mod.out$runout)
  if(!is.null(ref.model)){
    if(ref.model < 1 || ref.model > N) stop("ref.model number outside bounds.")
  } 
  ...
  for (i in seq(N)){
    ...
    if(align.global==TRUE)
      align <- solve_LSAP(ref.beta%*%t(betamod), maximum=TRUE)
    else{
