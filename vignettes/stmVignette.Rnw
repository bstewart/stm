...
\kappa_{y_d,k}^{(\text{i})})
\end{equation}
$m$, and each $\kappa_k^{(\text{t})}$, $\kappa_{y_d}^{(\text{c})}$ and $\kappa_{y_d,k}^{(\text{i})}$ are $V$-length vectors containing one entry per word in the vocabulary.  When no content covariate is present $\beta$ can be formed as $\beta_{d,k} \propto \text{exp}(m + \kappa_{k}^{(\text{t})})$ or simply point estimated (this latter behavior is the default). 
