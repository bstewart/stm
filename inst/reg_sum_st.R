...
for(k in ks) {
  ...
  deter <- stm(heldout$documents, heldout$vocab, K=k, init.type = "Spectral", max.em.its = 100, verbose = FALSE, control=list(method="BFGS")) 
  ...
  cat("randomized run\n")
  rand <- stm(heldout$documents, heldout$vocab, K=k, init.type = "Spectral", seed=13, max.em.its = 100, verbose = FALSE, control=list(method="BFGS", rand_docs=TRUE)) 
  num_topics <- cbind(num_topics, k)
...
save(reg_sum_st_df,file="reg_sum_st_df.Rda")